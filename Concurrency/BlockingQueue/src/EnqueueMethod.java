...
public class EnqueueMethod {
    ...
    public static void main(String[] args) {
        ...
        // add will throw IllegalException if there is no space available in BlockingQueue
        try {
            blockingQueue.add("2");
        } catch(IllegalStateException e) {
            System.out.println("No space inside BlockingQueue");
        }
        ...
        try {
            boolean wasEnqueued2 = blockingQueue.offer("3", 1000, TimeUnit.MICROSECONDS);
        } catch (InterruptedException e) {
            e.printStackTrace();
        }
    }
